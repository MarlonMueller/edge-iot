...
static SemaphoreHandle_t gps_data_received_sem;
...
void gps_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_id == GPS_UPDATE)
    {
        ...
        // Extract GPS data and store it in the provided array
        double *gps_data = (double *)event_handler_arg;

        gps_data[0] = gps->latitude;
        gps_data[1] = gps->longitude;
        gps_data[2] = gps->tim.hour;
        gps_data[3] = gps->tim.minute;
        gps_data[4] = gps->tim.second;
    ...
    else if (event_id == GPS_UNKNOWN)
    {
        ...
        ESP_LOGW(TAG, "GPS Unknown statement:%s", (char *)event_data);
    }
}

void get_gps_data(double *latitude, double *longitude, uint8_t *hour, uint8_t *minute, uint8_t *second, bool *valid)
{
    // Create a semaphore to signal when GPS data is received
    gps_data_received_sem = xSemaphoreCreateBinary();

    if (gps_data_received_sem == NULL)
    {
        ESP_LOGE(TAG, "Failed to create semaphore");
        return;
    }

    // Array to hold the variables for GPS data
    double gps_data[6];
    ...
    // Register event handler for NMEA parser library and pass the array as an argument
