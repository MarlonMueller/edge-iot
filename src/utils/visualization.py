...
def plot_history(assets_dir: pathlib.Path, model_name: str, history):
    ...
    ax2.plot(
        range(1, num_epochs + 1),
        np.round(np.array(training_accuracies) * 100),
    ...
    ax2.plot(
        range(1, num_epochs + 1),
        np.round(np.array(testing_accuracies) * 100),
...
def plot_confusion_matrices(
    ...
    for i, (dataset, dataset_name) in enumerate(zip(datasets, dataset_names)):
        ...
        cm = confusion_matrix(
            y_true, y_pred, labels=range(num_classes), normalize="all"
        )
...
def plot_quantized_confusion_matrices(
    assets_dir: pathlib.Path,
    ...
    for i, (dataset, dataset_name, dataset_size) in enumerate(
        zip(datasets, dataset_names, dataset_sizes)
    ):
        ...
        for (data, labels) in dataset.as_numpy_iterator():
            ...
            predictions_original.append(
                (labels.flatten(), np.argmax(prediction_original[0], axis=1))
            )

            # Quantized Model
            [prediction_quantized, _] = evaluator.evalute_quantized_model(data, False)
            predictions_quantized.append(
                (labels.flatten(), np.argmax(prediction_quantized[0], axis=1))
            )
        ...
        cm_original = confusion_matrix(
            np.concatenate(labels_original),
            np.concatenate(preds_original),
            labels=range(len(class_names)),
            normalize="all",
        )

        cm_quantized = confusion_matrix(
            np.concatenate(labels_quantized),
            np.concatenate(preds_quantized),
            labels=range(len(class_names)),
            normalize="all",
        )

        # Plot floating-point confusion matrix
        sns.heatmap(
            cm_original,
            annot=True,
            fmt=".0%",
        ...
        axes[i + 2].set_title(
            f"{dataset_name} data fixed-point ({quantization_bit})", fontsize=16
        )

    fig.tight_layout()

    plt.savefig(
        assets_dir / f"cm_{model_name}_{quantization_bit}_original_vs_quantized.pdf"
    )
